...
Zunächst betrachten wir nur die Serien `City` und `Friends` und reduzieren daher die Datentabelle um die Beobachtungen für die `Theme` nicht `DUPLO` ist.
...
- Welche Variable (`Pieces`, `Amazon_Price`) sollte als erklärende Variable $x$, welche als zu erklärende Variable $y$ verwendet werden?
...
- Welchen modellierten Wert hat ein Set mit $x_j=10$ Beurteilen? Gilt immer $\hat{y}_j = y_j$?
